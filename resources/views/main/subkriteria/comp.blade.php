					...
					<table class="table table-lg table-hover table-striped text-center">
						...
						<tbody>@csrf
							...
							<tr>
								<td>{{ ++$numindex }}</td>
								<th>
									<input type="radio" class="btn-check" name="subkriteria[{{ $loop->index }}]" id="left-{{ $loop->index }}" value="left" autocomplete="off"
										{{$value[$loop->index]['nilai'] > 0 || old("subkriteria.$loop->index") == 'left' ? 'checked' : '' }} required>
