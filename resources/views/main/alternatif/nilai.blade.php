...
@section('content')
<div class="modal fade text-left" id="NilaiAlterModal" tabindex="-1" role="dialog" aria-labelledby="NilaiAlterLabel" aria-hidden="true" data-bs-focus="false">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="POST" enctype="multipart/form-data" id="NilaiAlterForm" class="needs-validation">
					...
					<div class="input-group has-validation mb-3" data-bs-toggle="tooltip" data-bs-placement="right" title="{{ $kr->name }}">
						<label class="input-group-text" for="subkriteria-{{ Str::slug($kr->name,'-') }}">
							C{{ $kr->id }}
						</label>
						<select class="form-select" id="subkriteria-{{ Str::slug($kr->name,'-') }}" name="subkriteria_id[]" required>
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		try {
			...
			nilaialtdt = nilaialtdt.DataTable({
				...
				@foreach($data['kriteria'] as $kr)
				{orderable: false, targets: 1 + {{ $loop->index }} },
				@endforeach
				{
					orderable: false,
					targets: -1,
		...
		confirm.fire({
			titleText: 'Hapus nilai alternatif?',
		...
		formloading('#NilaiAlterForm :input',true);
		...
		}).always(function () {formloading('#NilaiAlterForm :input',false);});
	});
	function submitform(event) {
		...
		$.ajax({
			...
			type: 'POST',
			beforeSend: function () {
				$('#NilaiAlterForm :input').removeClass('is-invalid');
				$('#NilaiAlterModal :button').prop('disabled', true);
				formloading('#NilaiAlterForm :input',true);
			},
			complete: function () {
				$('#NilaiAlterModal :button').prop('disabled', false);
				formloading('#NilaiAlterForm :input',false);
			},
			success: function (status) {
				if ($.fn.DataTable.isDataTable("#table-nilaialt")) nilaialtdt.draw();
				$('#NilaiAlterModal').modal('hide');
				swal.fire({	icon: "success",	titleText: "Berhasil dinilai"	});
			},
			error: function (xhr, st) {
				if (xhr.status === 422) {
					resetvalidation();
