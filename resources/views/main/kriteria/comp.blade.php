...
@php($numindex = 0)
<div class="card">
	<div class="card-header"><h4 class="card-title">Masukkan Perbandingan</h4></div>
		...
		<div class="card-body">
			<div class="accordion mb-3" id="accordionTabelPerbandingan">
				<div class="accordion-item">
					...
					<div id="flush-collapseOne" class="accordion-collapse collapse" data-bs-parent="#accordionTabelPerbandingan">
			...
			<div class="table-responsive">
				<form method="POST" enctype="multipart/form-data" action="{{ route('bobotkriteria.store') }}">
					<table class="table table-lg table-hover table-striped text-center">
						<thead>
							<tr>
								<th>No</th><th>Kriteria</th><th>Perbandingan</th><th>Kriteria</th>
							</tr>
						</thead>
						<tbody>@csrf
							...
							<tr>
								<td>{{ ++$numindex }}</td>
								<th>
									<input type="radio" class="btn-check" name="kriteria[{{ $loop->index }}]" id="left-{{ $loop->index }}" value="left" autocomplete="off"
								...
								<td>
									<div class="input-group mb-3">
										<input type="number" name="skala[{{ $loop->index }}]" min="1" max="9" class="form-control text-center @error('skala.' . $loop->index) is-invalid @enderror " value="{{ old('skala.' . $loop->index) ?? (abs($value[$loop->index]['nilai']) ?? '') }}" required>
...
<script type="text/javascript">
	$(document).on('click', '#reset-button', function (e) {
		...
		confirm.fire({
			...
			if (result.isConfirmed) $('#reset-kriteria').submit();
		});
	});
</script>
@endsection